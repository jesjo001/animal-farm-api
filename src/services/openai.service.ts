...
export class OpenAIService {
   ...
   */
  static async analyzeChickAudio(
   ...
   */
  private static async extractAudioFeaturesWithWhisper(audioFilePath: string) {
    try {
   ...
   */
  private static async classifyWithGPT4(
    audioFeatures: any,
    trainingContext?: string
  ): Promise<any> {
   ...
   */
  private static buildSystemPrompt(trainingContext?: string): string {
...
`;
   ...
   */
  private static buildUserPrompt(audioFeatures: any): string {
...
Based on these features, classify the chick as male or female with your confidence level.`;
   ...
   */
  private static estimatePitchFromSegments(segments: any[]): number {
    if (!segments || segments.length === 0) return 2500; // Default mid-range
   ...
   */
  private static estimateNoiseLevel(segments: any[]): number {
   ...
   */
  private static analyzeEnergyPattern(segments: any[]): number {
    if (!segments || segments.length < 2) return 0.5;
   ...
   */
  private static calculateFrequencyVariation(segments: any[]): number {
    if (!segments || segments.length < 2) return 0.5;
   ...
   * Fallback: Basic audio analysis without Whisper
   */
  private static async basicAudioAnalysis(audioFilePath: string): Promise<any> {
   ...
   */
  static async generateTrainingContext(tenantId: string): Promise<any> {
    const validatedSamples = await TrainingSampleModel.find({
      tenantId,
      isValidated: true
